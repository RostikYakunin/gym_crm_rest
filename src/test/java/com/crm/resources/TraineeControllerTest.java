...
class TraineeControllerTest extends UnitTestBase {
    ...
    @CsvSource({
            ...
            "user2, oldPas2, newPas2, false, 400, 'Password was not changed'"
    })
    ...
    void shouldChangePasswordSuccessfully(
        ...
        when(traineeService.findByUsername(anyString())).thenReturn(testFoundTrainee);
        ...
        verify(traineeService, times(1)).findByUsername(stringArgumentCaptor.capture());
    ...
    void shouldDeleteTraineeSuccessfully(String username, boolean traineeExists, int expectedStatus) throws Exception {
        ...
        if (traineeExists) {
            ...
            String expectedMessage = "Trainee with userName=" + username + " was deleted";
            ...
            verify(traineeService, times(1)).delete(foundTrainee);
        } else {
            assertThrows(
    ...
    @DisplayName("Should update/not update trainee`s profile according to data")
    void shouldUpdateTraineeTrainings(String username, boolean traineeExists, boolean validTrainings, int expectedStatus) throws Exception {
        ...
        lenient().when(traineeService.findByUsername(anyString())).thenReturn(foundTrainee);
        ...
        if (traineeExists) {
            ...
            verify(traineeService, never()).findByUsername(stringArgumentCaptor.capture());
    ...
    @CsvSource({
            ...
            "user3, true, 404, 'Trainee not found'"
    })
    ...
    void shouldUpdateTraineeStatusWithValidData(String username, boolean isActive, int expectedStatus, String expectedMessage) throws Exception {
        ...
        if (expectedStatus == 404) {
            when(traineeService.findByUsername(username)).thenReturn(null);
        } else {
            when(traineeService.findByUsername(username)).thenReturn(testTrainee);
            if (isActive) {
